...
const insertStudent = async (student: Student) => {
  await dbQuery(`INSERT INTO students(id, name, birthday, birth_place, nationality, cpf) VALUES(?, ?, ?, ?, ?, ?)`,
...
const getWorkloadPending = async (course_id: number, period_init: string, period_end: string) => {
  const retorno = await dbQuery(`SELECT COALESCE(mc, 0) AS mc, (required_component_workload - fulfilled_component_workload) AS PendingWorkload FROM curriculums WHERE course_id = ? 
...
export const getBoxPlot = async (course_id: number): Promise<BoxPlot[]> => {
  ...
  return retorno as BoxPlot[]; // Retorna todos os resultados como um array de BoxPlot
};

const getViolinPlot = async (course_id: number, period_init: string, period_end: string): Promise<ViolinPlot[]> => {
  const retorno = await dbQuery(`SELECT 
    c.initial_period, 
...
const updateStudent = async (student: Student) => {
  await dbQuery(`UPDATE students SET name = ?, birthday = ?, birth_place = ?, 
  nationality = ?, cpf = ? WHERE id = ?`, [student.name, student.birthday, student.birth_place, student.nationality, student.cpf, student.id])
}
...
const getSankeyData = async (period_init: string, period_end: string) => {
  const sankeyQuery = `
    SELECT 
  ...
  const retorno = await dbQuery(sankeyQuery, [period_init, period_end]);
  return retorno;
}
