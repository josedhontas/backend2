...
const insertStudent = (req: Request, res: Response) => {

  {
    ...
    if (!student)
      return badRequest(res, "estudante invalido!")
...
const updateStudent = (req: Request, res: Response) => {
  const id = parseInt(req.params.id)
  const studentExist = studentModel.getStudent(id)
  if (!studentExist)
    return notFound(res)
...
export const getSankeyData = async (req: Request, res: Response) => {
  try {
    const { period_init, period_end } = req.params;
    const data = await studentModel.getSankeyData(period_init, period_end);
    res.status(200).json(data);
  } catch (error) {
    res.status(500).json({ error: 'Erro ao obter dados do Sankey' });
  }
};
